...
Component({
  ...
  methods: {
    toggle() {
      if (this.data.open) {
        notepad.save(this.data.part, String(this.noteText || ""));
        this.setData({ text: String(this.noteText || ""), open: false });
        return;
      }
      this.noteText = notepad.load(this.data.part);
      this.setData({ text: this.noteText, open: true });
    },
