...
def main() -> None:
    ...
    try:
        ...
        evaluate(browser, "(() => { document.documentElement.classList.add('pwa-keyboard-open'); const home=document.querySelector('.home-shell').getBoundingClientRect(); const editor=document.querySelector('[data-note]').getBoundingClientRect(); document.querySelector('.candidate-overlay').style.setProperty('--keyboard-candidate-top', `${Math.round(editor.bottom-home.top-1)}px`); return true; })()")
        keyboard_layout = evaluate(browser, "new Promise(resolve => requestAnimationFrame(() => { const editor=document.querySelector('[data-note]').getBoundingClientRect(); const candidate=document.querySelector('.candidate-overlay').getBoundingClientRect(); const header=document.querySelector('.home-header').getBoundingClientRect(); resolve({editorHeight:editor.height, editorBottom:editor.bottom, candidateTop:candidate.top, candidateHistories:[...document.querySelectorAll('.candidate-history')].filter(item => getComputedStyle(item).display !== 'none').length, columns:getComputedStyle(document.querySelector('.candidate-sets')).gridTemplateColumns, header:getComputedStyle(document.querySelector('.home-header')).display, headerHeight:header.height, rail:getComputedStyle(document.querySelector('.theme-strip')).display, archive:getComputedStyle(document.querySelector('.theme-archive')).display}); }))")
        ...
        assert len(keyboard_layout["columns"].split()) == 2, keyboard_layout
