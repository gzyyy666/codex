...
def main() -> None:
    ...
    assert 'if (action === "expand-note") { state.noteExpanded = true; state.shareDraft = state.note; state.shareTitle = "手机训练记录"; state.shareConfirmReady = false;' in app_source
    assert '再次确认并发送' in app_source and 'PHONE_INBOX_TIMEOUT_MS = 15000' in app_source
    ...
    assert 'fitness-ledger-pwa-v31' in service_worker
    assert '"./data-modules.js?v=20260820-04"' in service_worker
    assert 'register("./sw.js?v=20260826-01", { updateViaCache: "none" })' in app_source
