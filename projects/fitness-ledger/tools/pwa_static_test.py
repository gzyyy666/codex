...
def main() -> None:
    ...
    assert 'register("./sw.js?v=20260914-01", { updateViaCache: "none" })' in app_source
    ...
    assert 'html[data-pwa-keyboard="open"] .reference-home .note-stack:not(.note-stack--compact) .note-editor' in css_source
    assert 'min-height: 320px;' in css_source
    ...
    assert 'function keyboardViewport()' in app_source and 'pwaKeyboard' in app_source
