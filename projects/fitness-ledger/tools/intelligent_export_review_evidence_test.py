...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="fitness-ledger-review-evidence-") as name:
        ...
        adapter = FakeLocalModelAdapter([plan_for(views)])
        ...
        assert evidence["query_scope"]["target_body_part_ids"] == ["SHOULDER"]
        assert evidence["intent"]["target_body_parts"] == []
        ...
        bad_scope["query_scope"]["date_request"] = "invalid"
        blocked_intent = build_bundle([("review", "比较最近肩部训练、饮食和体重变化", bad_scope)])
        assert "REVIEW_DATE_SCOPE_INVALID" in blocked_intent["integrity_audit"]["blocking_integrity_codes"]
        ...
        repair_adapter = FakeLocalModelAdapter([plan_for(views, invalid=True), plan_for(views)])
