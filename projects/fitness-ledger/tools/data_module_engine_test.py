...
class DataModuleCandidateTests(unittest.TestCase):
    ...
    def test_template_statistics_and_release_readiness_are_local_and_bounded(self) -> None:
        ...
        self.assertEqual(template["schema"], "fitness-ledger-llm-entry-template-v4")
        self.assertEqual(template["template_version"], 4)
        self.assertIn("训练：肩部", template["prompt_template"])
        self.assertIn(" 1. 悍马推肩", template["prompt_template"])
        self.assertIn("没有明确有氧内容", template["prompt_template"])
        self.assertIn("排便：待补充", template["prompt_template"])
        self.assertIn("饮食备注：", template["prompt_template"])
        self.assertIn("标准字段、动作名称、食物名称、单位、备注词和有氧词", template["prompt_template"])
