...
const BUILD_VERSION = "PWA v1.1.32 · build 2026.09.17.08";
...
function sessionRelationDetails(relations, item) {
  if (!relations.length) return "";
  return relations.map(relation => {
    const members = Array.isArray(relation.co_members) ? relation.co_members : [];
    const memberCount = Number(relation.member_count || members.length + 1);
    const currentOrder = relation.member_order || "";
    const currentName = item?.movement_name || item?.movement_id || "当前动作";
...
function renderRecord() {
  ...
  if (mode) {
    ...
    const movementMarkup = movements.length ? movements.map((item, index) => {
      ...
      const relationDetails = sessionRelationDetails(relations, item);
...
if ("serviceWorker" in navigator) navigator.serviceWorker.register("./sw-caret-gap-page-blank.js", { updateViaCache: "none" }).catch(() => {});
