...
function renderRecord() {
  ...
  if (mode) {
    ...
    const movementMarkup = movements.length ? movements.map((item, index) => {
      ...
      const content = `<span class="movement-order">第 ${esc(order)} 个动作</span><strong>${esc(name)}</strong>${item.english_name ? `<small class="movement-english">${esc(item.english_name)}</small>` : ""}${relations.length ? `<span class="session-relation-badge">超级组${relations[0].label ? ` ${esc(relations[0].label)}` : ""}</span>` : ""}<div class="movement-summary-block"><span class="eyebrow">MOVEMENT SUMMARY / 本次摘要</span><p>${esc(item.summary || setSummary(item))}</p></div>${item.notes ? `<div class="movement-notes-block"><span class="eyebrow">NOTES / 动作备注</span><p>${esc(item.notes)}</p></div>` : ""}${item.movement_id ? `<span class="training-action">查看动作轨迹 →</span>` : `<span class="training-action muted">自定义动作记录</span>`}`;
      return item.movement_id ? `<button class="training-action-card session-movement-card" data-action="movement" data-id="${esc(item.movement_id)}">${content}</button>` : `<article class="training-action-card session-movement-card">${content}</article>`;
