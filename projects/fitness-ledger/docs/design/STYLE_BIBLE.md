...
The Body archive may precede its slips with one low-height ink calendar field. Each cell is one real calendar day inside the recent 90-day window; only recorded weigh-in days receive a small height-coded mark, while missing dates remain visibly empty. Muted terracotta, teal, and graphite distinguish increase, decrease, and flat change from the previous weigh-in without turning the chart into a status palette. Month rails, scale labels, focused values, and peak annotations must remain honest. The field reveals on entry and supports an explicit replay, but never loops.
...
The recent progression chart uses the Lupi Basics `F8 Plumb Scatter` grammar for a sparse, chronological session field. X reads effective session order/date, Y reads maximum load or total repetitions, dot area reads relative workload, and the plumb stem returns each record to a shared floor. The focused value scale must be labeled; peak and latest sessions receive restrained emphasis. The chart reveals on entry and supports an explicit replay, with a reduced-motion fallback; it must not loop, interpolate missing sessions, or include excluded history to manufacture a stronger trend.
Selection audit: `F8 Plumb Scatter` remains appropriate for sparse session progress because it keeps date order, load, workload, and set count legible without implying a continuous trend. Body now uses a calendar matrix instead of a line, lollipop, or area chart: the fixed day cells make missing weigh-ins explicit and leave color available for a restrained change cue.
...
The plot is the primary subject of both archive pages. The chart shell therefore gives the SVG the widest column and a tall reading field; copy and latest-value summaries remain quiet margins. Body uses a fixed 90-day calendar field with month ticks and empty rails, while Movement keeps equal spacing for sparse effective sessions. A single Movement session is labelled as one recorded session rather than a trend; two sessions are presented as a comparison; three or more may add first/peak/latest hierarchy without connecting the marks.
