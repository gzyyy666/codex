...
AnalysisExportRequest v1.1 materializer. It opens the two explicit formal files
read-only and delegates validation, resolution, time-window handling,
projection, and export rendering to the accepted deterministic materializer.
"""
...
def _require_file(path: str | Path, label: str) -> Path:
    ...
    if not resolved.is_file():
        raise FormalReadOnlyDataSourceError(
            f"{label} must be an existing regular file: {resolved}"
        )
    return resolved
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise FormalReadOnlyDataSourceError(
            f"Unable to read {label} as UTF-8 JSON: {path}"
        ) from error
...
class FormalReadOnlyDataSource:
    ...
    def __init__(
        ...
        movement_dictionary_path: str | Path,
    ) -> None:
        self.tracker_path = _require_file(tracker_path, "tracker.json")
        self.movement_dictionary_path = _require_file(
        ...
        self.snapshot_id = "formal-readonly-" + hashlib.sha256(
            _canonical(
                {key: value["sha256"] for key, value in self._before.items()}
            ).encode("utf-8")
        ).hexdigest()[:24]
        fixture["fixture_version"] = (
            "anonymous-formal-readonly-" + self.snapshot_id.rsplit("-", 1)[-1]
        )
        fixture["generated_at"] = max(
            datetime.fromtimestamp(
                value["modified_time_ns"] / 1_000_000_000,
                tz=timezone.utc,
            )
    ...
    def _to_materializer_fixture(
        ...
        if not all(
            ...
            for items in (daily, diet, training, dictionary_items)
        ):
            raise FormalReadOnlyDataSourceError(
                "Formal structured collections must be arrays"
            )
        if not isinstance(movements, dict):
            raise FormalReadOnlyDataSourceError(
                "Formal movement histories must be an object"
            )

        body_rows = [
            _mapped_record(item, FormalReadOnlyDataSource._BODY_FIELDS)
            for item in daily
            if isinstance(item, dict)
        ]
        diet_rows = [
            _mapped_record(item, FormalReadOnlyDataSource._DIET_FIELDS)
            for item in diet
            if isinstance(item, dict)
        ]
        training_rows = [
            ...
            movement_catalog.append(
                {
                    ...
                    "movement_name": deepcopy(
                        item.get("display_name") or item.get("english_name") or ""
                    ),
            ...
            for history in movement.get("history", []):
                ...
                row: dict[str, Any] = {
                    ...
                    "movement_name": deepcopy(
                        ...
                        or catalog_item.get("english_name")
        ...
        if not dates:
            raise FormalReadOnlyDataSourceError(
                "Formal structured data contains no usable dates"
            )
        return {
    ...
    def _formalize_bundle(self, bundle: dict[str, Any]) -> dict[str, Any]:
        ...
        provenance.update(
            {
                ...
                "formal_access": (
                    "read_only; structured allowlist projection"
                ),
                "source_path_policy": (
                    "explicit formal files opened read-only; "
                    "structured allowlist projection only"
                ),
                "materializer_version": MATERIALIZER_VERSION,
            }
        )
        for dataset in result["request"]["datasets"]:
            if dataset.get("set_roles") and any(
                ...
                and item.get("sets")
                and any("role" not in set_item for set_item in item["sets"])
                for item in self._materializer.fixture["datasets"][
                    "movement_progress"
                ]
            ):
                result["warnings"].append(
                    f"{dataset['dataset_id']}: formal set-role metadata is "
                    "unavailable; no set-role inference was applied"
                )
    ...
    def materialize_with_exports(
        ...
        if "markdown" in formats:
            exports["markdown"] = (
                AnonymousFixtureMaterializer.export_markdown(bundle)
            )
    ...
    def resolve(self, selector: dict[str, str]) -> list[dict[str, Any]]:
        return [
            deepcopy(item)
            for item in self._materializer._movement_matches(selector)
        ]
